...
public class ImportService : IImportService
{
    ...
    private static readonly Dictionary<string, string[]> EmployeeHeaderAliases = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        {
            "LastName",
            new[] { "lastname", "last_name", "surname", "Lastname", "prénome", "prenom", "Prenom", "Prénom" }
        },
        ...
        { "IsActive", new[] { "isactive", "active", "is_active", "travaille" } },
        {
            ...
            new[]
            {
                "departmentid", "department_id", "deptid", "dep_id", "id_department", "idDepartement", "departementId"
            }
        },
        {
            "DepartmentCode",
            new[] { "departmentcode", "department_code", "deptcode", "dept_code", "codeDepartement", "departementCode" }
        },
        {
            "DepartmentName",
            new[] { "departmentname", "department_name", "deptname", "nomDepartement", "nom du departement" }
        }
    };
    ...
    private static readonly Dictionary<string, string[]> DepartmentHeaderAliases = new(StringComparer.OrdinalIgnoreCase)
    {
        {
            "Name",
            new[] { "name", "departmentname", "department_name", "deptname", "nomDepartement", "nom du departement" }
        },
        {
            "Code",
            new[]
            {
                "code", "departmentcode", "department_code", "deptcode", "dept_code", "codeDepartement",
                "departementCode"
            }
        },
            ...
            while (csv.Read())
            {
                ...
                try
                {
                    ...
                    var dto = new EmployeeCreateDto
                    {
                        ...
                        Salary = decimal.TryParse(GetField("Salary"), NumberStyles.Any, CultureInfo.InvariantCulture,
                            out var s)
                            ? s
                            : 0m,
                    ...
                    if (existing != null)
                    {
                        ...
                        {
                            await EnsureAndMapDepartmentAsync(dto, existing, departmentRepository, mapper,
                                cancellationToken);
                    ...
                    {
                        int assignedDeptId =
                            await ResolveOrCreateDepartmentForDtoAsync(dto, departmentRepository, mapper,
                                cancellationToken);
                ...
                catch (Exception ex)
                {
                    result.Failed++;
                    result.Records.Add(new ImportRecordResultDto
                        { RowNumber = row, Success = false, Message = ex.Message });
                }
            }
        }
                ...
                for (int r = 2; r <= lastRowUsed.RowNumber(); r++)
                {
                    ...
                    try
                    {
                        ...
                        var dto = new EmployeeCreateDto
                        {
                            ...
                            Salary = decimal.TryParse(GetByCanonical("Salary"), NumberStyles.Any,
                        ...
                        if (string.IsNullOrWhiteSpace(dto.Email))
                        {
                            result.Failed++;
                            result.Records.Add(new ImportRecordResultDto
                                { RowNumber = row, Success = false, Message = "Missing email" });
                            continue;
                        }
                        ...
                        {
                            var assignedDeptId = await ResolveOrCreateDepartmentForDtoAsync(dto, departmentRepository,
    ...
    private async Task<int> ResolveOrCreateDepartmentForDtoAsync(EmployeeCreateDto dto, IDepartmentRepository deptRepo,
        IMapper mapper, CancellationToken cancellationToken)
    {
        if (dto.DepartmentId.HasValue && dto.DepartmentId.Value > 0)
        {
            ...
            {
                Name = string.IsNullOrWhiteSpace(dto.DepartmentName)
                    ? $"Imported Dept {dto.DepartmentId.Value}"
                    : dto.DepartmentName,
                Code = string.IsNullOrWhiteSpace(dto.DepartmentCode)
                    ? $"IMP-{dto.DepartmentId.Value}"
                    : dto.DepartmentCode,
        ...
        {
            var byCode =
                (await deptRepo.FindAsync(d => d.Code == dto.DepartmentCode, cancellationToken)).FirstOrDefault();
        ...
        {
            var byName =
                (await deptRepo.FindAsync(d => d.Name == dto.DepartmentName, cancellationToken)).FirstOrDefault();
    ...
    private async Task EnsureAndMapDepartmentAsync(EmployeeCreateDto dto, Employee existing,
        IDepartmentRepository deptRepo, IMapper mapper, CancellationToken cancellationToken)
    {
        if (dto.DepartmentId.HasValue && dto.DepartmentId.Value > 0)
        {
            var dep = await deptRepo.GetByIdAsync(dto.DepartmentId.Value, cancellationToken);
        ...
        if (!string.IsNullOrWhiteSpace(dto.DepartmentCode))
        {
            ...
            if (byCode != null)
            {
                existing.DepartmentId = byCode.Id;
                return;
            }
        ...
        if (!string.IsNullOrWhiteSpace(dto.DepartmentName))
        {
            ...
            if (byName != null)
            {
                existing.DepartmentId = byName.Id;
                return;
            }
    ...
    public async Task<ImportResultDto> ImportDepartmentsAsync(Stream fileStream, string fileName,
        ImportDuplicateBehavior behavior = ImportDuplicateBehavior.Update,
        CancellationToken cancellationToken = default)
    {
        var result = new ImportResultDto();
        ...
        switch (ext)
        {
            case ".csv":
            {
                ...
                while (await csv.ReadAsync())
                {
                    ...
                    try
                    {
                        ...
                        var existing = (await departmentRepository.FindAsync(d => d.Code == dto.Code || d.Name == dto.Name,
                                cancellationToken))
                            .FirstOrDefault();
            ...
            case ".xlsx" or ".xls":
            {
                ...
                {
                    for (var c = 1; c <= lastHeaderCell2.Address.ColumnNumber; c++)
                    {
                        var h = headerRow.Cell(c).GetString().Trim();
                        if (!string.IsNullOrEmpty(h)) headers.TryAdd(h, c);
                    }
                }
                ...
                {
                    for (var r = 2; r <= lastRowUsed2.RowNumber(); r++)
                    {
                        result.Total++;
                        try
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(dto.Name))
                            {
                                result.Failed++;
                                result.Records.Add(new ImportRecordResultDto
                                    { RowNumber = r, Success = false, Message = "Missing name" });
                                continue;
                            }

                            var existing =
                                (await departmentRepository.FindAsync(d => d.Code == dto.Code || d.Name == dto.Name,
                                    cancellationToken))
                                .FirstOrDefault();
                            ...
                            if (existing != null)
                            {
                                ...
                                result.Updated++;
                                result.Records.Add(new ImportRecordResultDto
                                    { RowNumber = r, Success = true, Message = "Updated" });
                            }
                            else
                            {
                                var entity = mapper.Map<Department>(dto);
                                ...
                                result.Created++;
                                result.Records.Add(new ImportRecordResultDto
                                    { RowNumber = r, Success = true, Message = "Created" });
                            }
                        }
                        catch (Exception ex)
                        {
                            result.Failed++;
                            result.Records.Add(new ImportRecordResultDto
                                { RowNumber = r, Success = false, Message = ex.Message });
                        }
                    }
                }
                else
